...
- **(fix) Глава 6: Орфографія, граматика та милозвучність (2 раунди)** ✅
  - (fix) **Раунд 1: Пунктуація та орфографія**
    * Заголовок: ТИПИ(ВИДИ) → ТИПИ (ВИДИ) (пробіл перед дужкою)
    * Граматика: 15% строк → 15% рядків (у програмуванні — рядок, не строка)
    * Дієслово: користувач видит → користувач бачить
    * Прикметник: 200 ms (хорошо) → 200 ms (добре)
    * Перевірено: вузьке місце (правильно), на відміну від (правильно)

  - (fix) **Раунд 2: Mermaid діаграма + милозвучність**
    * **Mermaid діаграма:**
      - «тестування функціональних можливостей» → «Тестування...» (велика літера)
      - «Нових функціональностей» → «Нових функціональних можливостей» (правильна множина)
      - «За автоматизованістю» → «За ступенем автоматизації» (професійна термінологія)
    * **Текст:**
      - «Вибери способ» → «спосіб», «виконий» → «виконаний», «Все функції» → «Усі функції»
    * **Милозвучність (У/В, І/Й):**
      - «Локалізація й інтернаціоналізація» → «та інтернаціоналізація» (після голосної)
      - «мови й культури» → «та культури» (після голосної)

  - (fix) **Якість:** 100% мовна коректність і професійна термінологія
...
- **(fix) Глава 5: Виправлено всі русизми та кальки (2 раунди)** 🔴
  - (fix) **Раунд 1: 16 базових мовних помилок**
    * Конструкції: «будується на основі» → «грунтується на», «залежності від» → «залежно від»
    * Термінологія ІСТQВ: «Білий ящик» → «біла скринька», «Чорний ящик» → «чорна скринька»
    * Граматика: «помилливе» → «помилкове», «всі добре» → «все добре»
    * Російські кальки: «направлення» → «скерування», «достут» → «доступ»
    * Лексика: «базується на» → «грунтується на», «Часовий етап» → «етап»
  - (fix) **Раунд 2: 25+ додаткових помилок**
    * Кальки: «на базі вимог» → «на основі вимог», «пошук по латин» → «пошук за латин»
    * Лексика: «Дисконт» → «Знижка», «Улегшує» → «Полегшує», «этап» → «етап»
    * Описки: «в чинні» → «вчасно», «не тісто» → «це не так»
    * Граматика: «описи функціональності» → «опис функціональності»
    * Капіталізація та структура: уніфіковано всі заголовки та маркери
