...
[![Photo of Sarang Sanjay Kulkarni](https://martinfowler.com/articles/reliable-llm-bayer/sarang.png)](https://www.linkedin.com/in/sarangk90/)
...
an [O’Reilly
course](https://www.oreilly.com/live-events/building-reliable-rag-applications-from-poc-to-production/0642572012347/) on building production-ready RAG applications.

## Contents

* [The Challenge: Navigating the Preclinical Data Maze](#TheChallengeNavigatingThePreclinicalDataMaze)
* [The Solution: PRINCE - An Evolutionary Platform](#TheSolutionPrince-AnEvolutionaryPlatform)
* [System Architecture: Engineering a Reliable Agentic RAG System](#SystemArchitectureEngineeringAReliableAgenticRagSystem)
* [The Agentic RAG System](#TheAgenticRagSystem)
  + [Clarify User Intent](#ClarifyUserIntent)
  + [Think & Plan: Process Reflection](#ThinkampPlanProcessReflection)
  + [The Researcher Agent](#TheResearcherAgent)
  + [The Reflection Agent: Data Validation and Sufficiency](#TheReflectionAgentDataValidationAndSufficiency)
  + [The Writer Agent: Answer Synthesis and Formatting](#TheWriterAgentAnswerSynthesisAndFormatting)
* [Building Trust in a Production LLM System](#BuildingTrustInAProductionLlmSystem)
  + [Transparency and Explainability](#TransparencyAndExplainability)
  + [Evaluation](#Evaluation)
  + [Monitoring](#Monitoring)
* [Engineering for Resilience: Error Handling and Recovery](#EngineeringForResilienceErrorHandlingAndRecovery)
* [Enhancing Data Quality: Named Entity Recognition and Annotation](#EnhancingDataQualityNamedEntityRecognitionAndAnnotation)
* [The Journey Continues: Iterative Development](#TheJourneyContinuesIterativeDevelopment)
...
combining the generative power of LLMs with the precision of information retrieval systems, [Retrieval-Augmented Generation](/articles/gen-ai-patterns/#rag) (RAG) has emerged as a promising technique.
...
published in [Frontiers in Artificial Intelligence](https://www.frontiersin.org/journals/artificial-intelligence/articles/10.3389/frai.2025.1636809/full) covers the
...
1. Search: the initial phase focused on creating a unified gateway to
...
2. Ask: this phase introduced an AI-powered question-answering system utilizing
...
3. Do: the current phase positions PRINCE as an active research assistant capable of
...
accurate, context-rich answers, is orchestrated using *LangGraph* and served via a *FastAPI* application.

[Figure 1](#system-container-view.png) provides the system context—UI, backend, data
stores, LLM fallbacks, and observability—while [Figure 2](#new-research-workflow.png)
...
+ The *state* of the agent's execution is meticulously tracked. After each logical
  step (a LangGraph node execution), the corresponding state is persisted in *PostgreSQL* using a *LangGraph checkpointer*.
...
PRINCE incorporates an agentic [RAG](/articles/gen-ai-patterns/#rag) system ([Figure 2](#new-research-workflow.png)) to handle complex user requests that require multiple
...
![](https://martinfowler.com/articles/reliable-llm-bayer/new-research-workflow.png)
...
the next steps before taking action—a technique inspired by [Anthropic's Think tool](https://www.anthropic.com/engineering/claude-think-tool).
...
a [hybrid retriever](/articles/gen-ai-patterns/#hybrid-retriever) approach focused on two distinct
patterns:
...
embedded and indexed in [Amazon OpenSearch Service](https://aws.amazon.com/opensearch-service/),
...
* Metadata Filter Generation: concurrently, the LLM generates a
  metadata filter based on the query. For example, a filter eq(study\_id, T123456-2) is
...
* Query Expansion: to ensure comprehensive retrieval and account for variations in
  phrasing and terminology, [query expansion](/articles/gen-ai-patterns/#query-rewrite) (multi
...
* Hybrid Retriever: information retrieval from the vector database ([Amazon OpenSearch
  Service](https://aws.amazon.com/opensearch-service/)) utilizes a Hybrid Search approach that combines metadata filtering,
...
+ Metadata Filtering: the metadata filter generated in the previous step
  (e.g., eq(study\_id, T123456-2)) is applied directly to the vector database query.
...
* Reranking: the initial set of retrieved chunks (k=~20) is then refined using a [Rerank](/articles/gen-ai-patterns/#reranker) step. A cross-encoder model (bge-reranker-large)
...
numerical assay results including dosage groups. As shown in the *Researcher Agent* can intelligently decide to hand over such queries to the
Text-to-SQL tool.

![](https://martinfowler.com/articles/reliable-llm-bayer/text-to-sql.png)
...
* SQL Query Generation and Validation: a model with strong code generation
  ...
  accuracy.
* Query Execution and Result Limiting: the validated SQL query is executed
  ...
  records at a time.
* Error Handling and Iteration: if the SQL query execution is successful, the
...
While the *Think & Plan* step provides process reflection, the *Reflection
...
from whether the workflow itself is progressing correctly.

In multi-step agentic workflows, these two types of reflection serve different but
equally important
...
As illustrated in the research workflow diagram ([Figure 2](#new-research-workflow.png)), after initial information retrieval and 'think
...
Once the Researcher Agent has collected the relevant evidence from RAG and Text-to-SQL,
...
The Writer Agent operates with a few non-negotiable rules. It must ground every claim in
...
For more complex responses—such as multi-section summaries or partially filled regulatory
...
This gives PRINCE three complementary reflection loops. Process reflection checks whether
...
Together, these agents form a practical context engineering pattern. The system does not
...
Building and maintaining user trust is paramount for the successful
...
Ensuring transparency and explainability is a critical aspect of PRINCE's
...
* Intermediate Steps and Transparency: given the iterative nature of the workflow
  ...
  information was shortlisted and used to formulate the final response.
* Factuality Verification through Citation: the system facilitates user
  ...
  simplifies the human review process.

### Evaluation

Rigorous evaluation is fundamental to building and maintaining a reliable
...
* Dataset Evaluations: conducted whenever significant changes are made to the core
  ...
  generated response against the reference answer, yielding quantitative metrics such as *Faithfulness* (degree to which the answer is supported by context), *Answer
...
* Live Traffic Evaluations: performed daily as a batch job on real user queries
  ...
  production, and understanding performance on diverse live queries.

### Monitoring

Continuous monitoring of the system's performance and outputs is essential
...
ensuring the reliability and safety of the system's responses.
...
requiring a complete restart of the entire workflow.
...
* State Persistence: the state of the entire workflow graph is persistently stored,
  ...
  crucial for achieving robustness in a stateful agentic system.
* Built-in Retries: the system is configured with built-in retries at various steps
...
* User-Initiated Retries: in addition to automated retries, users have the option
  ...
  attempt. This significantly improves user experience and saves computational resources.
* Framework-Level Support: the error recovery mechanisms are significantly
...
* LLM Fallbacks: to enhance reliability and mitigate issues related to model
  ...
  downtimes for external services are outside of our direct control.

This comprehensive approach to error handling and recovery minimizes the
...
essential for a production-ready system.

These mechanisms are harness engineering in practice. The LangGraph workflow acts as
...
To address this challenge and continuously enhance the quality of the
...
that should be represented in the structured metadata.

The process involves:

* Processing study PDFs to extract text and identify relevant entities (e.g.,
  study IDs, compound names, species, routes of administration, dosage
  information, clinical findings, etc.).
* Generating structured annotations based on the identified entities and their
  relationships within the text.

We are actively working on integrating this utility system into our data
...
PRINCE has been available to end-users since early 2024, with the agentic
...
In the initial stages, our focus was squarely on achieving the desired
...
the user experience or the quality of the results.

The development of PRINCE follows a continuous, iterative
...
Building a production-ready LLM application in a complex enterprise
...
from vast, previously inaccessible data repositories.

Our experience highlights the critical importance of focusing on
...
PRINCE has already shown promising results in enhancing data
...
The broader lesson from PRINCE is that production-ready agentic AI is not only about better
...
recoverable, and suitable for a regulated research environment.

As model capabilities improve, some parts of today's harness may become thinner or move
...
recovery, reflection, and verification remains essential.

We hope this overview provides valuable insights into the practical
considerations and technical depth required to build and productionise LLM
applications in a regulated and data-rich domain.

---

## Acknowledgments

The author gratefully acknowledges the invaluable contributions of Adam Zalewski, Annika
...
Subhashini Rajamani from the Thoughtworks team for their contributions to this work.

The author used AI assistance during the writing of this article. AI tools were used for
brainstorming ideas, creating outlines, and reviewing drafts to polish language and
improve clarity.

## Disclaimer

All activities described conform to Bayer's information classification, data governance,
