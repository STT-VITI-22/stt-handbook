...
Please note that this guide assumes you are familiar with k6 and already have performance tests. If you are new to performance testing or k6, we recommend looking at our
[get-started resources](/docs/k6/latest/get-started/resources/#learning).
...
[Automation into CI/CD pipelines](/docs/k6/latest/reference/integrations/#continuous-integration-and-continuous-delivery) is an option, but it’s not the only method to schedule the execution of performance tests. When creating a performance testing plan, it’s important to remember that there are different ways to run performance tests in a frequent basis:

* Cron and cron job runners.
* Cloud testing tools, such as [scheduling in Grafana Cloud k6](/docs/grafana-cloud/testing/k6/author-run/schedule-a-test/).
...
Your test suite should incorporate a diverse range of tests that can verify critical areas of your system using distinct
...
* [Smoke](/docs/k6/latest/testing-guides/test-types/smoke-testing/): Test for script errors and verify SUT with minimal traffic.
* [Average-Load](/docs/k6/latest/testing-guides/test-types/load-testing/): Test for regular/normal traffic.
* [Stress](/docs/k6/latest/testing-guides/test-types/stress-testing/): Test for maximum expected traffic.
* [Spike](/docs/k6/latest/testing-guides/test-types/spike-testing/): Test for a surge of traffic.
* [Soak](/docs/k6/latest/testing-guides/test-types/soak-testing/): Test for a prolonged period of traffic.
...
*To learn more about configuring workloads in k6, refer to
[Scenarios](/docs/k6/latest/using-k6/scenarios/#scenario-executors).*
...
As a general rule on pre-release environments, we should run our larger tests with quality gates, Pass/Fail criteria that validate SLOs or reliability goals. In k6, you can do that by using
[Thresholds](/docs/k6/latest/using-k6/thresholds/) in `options` as follows:
...
The first step is learning what options you have for outputting performance results. If you’re using k6, there are a few
[options you can choose from](/docs/k6/latest/results-output/). You can review those options and the
[k6 metrics](/docs/k6/latest/using-k6/metrics/) to decide on a long-term solution to analyze the results of your test automation plan.

Here are some questions to consider when creating your result analysis process.

### How to store your performance results

In k6, you can get the
[aggregated results](/docs/k6/latest/results-output/end-of-test/) at the end of a test, or
[time series metrics in real-time](/docs/k6/latest/results-output/real-time/). Both options allow you to customize the output.
...
k6 provides
[built-in metrics](/docs/k6/latest/using-k6/metrics/reference/) that aggregate all the interactions against the SUT. You can also utilize
[tags](/docs/k6/latest/using-k6/tags-and-groups/#user-defined-tags) and custom metrics to categorize and filter results for one interaction or particular types.
...
Additionally, we recommend setting up alerts for important issues. Think about priority and non-priority levels and follow-up actions. Consider these [tips to design alerts](/docs/grafana/latest/alerting/).
...
To bridge the gap, work out a way to correlate testing results with how you instrument your infrastructure and application code. For instance, connecting or building custom dashboards with test results or using
[trace data from your tests](/docs/k6/latest/javascript-api/jslib/http-instrumentation-tempo/).
...
To stop a k6 test, learn how to use the
[`abortOnFail` threshold option](/docs/k6/latest/using-k6/thresholds/#abort-a-test-when-a-threshold-is-crossed) or integrate with the k6 CLI or Grafana Cloud k6.
...
Quality gates in performance tests are often defined as
[Pass/Fail criteria](/docs/k6/latest/using-k6/thresholds/) that verify the release meets its reliability goals.
