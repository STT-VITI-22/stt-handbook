...
08 Oct 2019
by [Tyler Hoffman](/authors/tyler)
...
those interpreting accelerometer and gyroscope data. *All* of these items can be
...
This is the second post in our [Building Better Firmware series](/tag/better-firmware),
following the [post](/blog/continuous-integration-for-firmware)
...
* [Framework-less Unit Tests](#framework-less-unit-tests)
* [Minimal Unit Test Example](#minimal-unit-test-example)
* [Unit Testing Best Practices](#unit-testing-best-practices)
* [Stubs, Fakes, and Mocks](#stubs-fakes-and-mocks)
* [Real World Unit Test Example](#real-world-unit-test-example)
* [Setting Up CppUTest](#setting-up-cpputest)
* [Tips & Tricks](#tips--tricks)
* [Common Issues with C/C++ Unit Tests](#common-issues-with-cc-unit-tests)
...
Driven Development (TDD)[1](#fn:0), the time it takes to **stabilize** embedded
...
1. This post covers how to test embedded **software**. Testing firmware drivers
   and hardware is *very* different and time is best spent writing functional
...
* Many *checks*, such as `LONGS_EQUAL` which compares integer values and
...
test with *no* other dependencies. Firmware is naturally coupled with other
...
the future. If we are testing only the flash key/value store, we *do not* want
...
* Each test should be *quick*. A few milliseconds is ideal, and one second is
  the worst case run time.
* Each unit test should ideally include one *real* implementation of a module.
...
* **header\_overrides/** - headers that override complex, auto-generated, or
...
* [Fake Function Framework (fff)](https://github.com/meekrosoft/fff) - *Although
  the name suggests it generates fakes implementations, they are actually mocks*
...
its entirety, a filesystem by ARM designed for microcontrollers[4](#fn:4).
...
write this entire module *and* test it without actually using real hardware.
...
Thankfully, `littlefs` includes an *emulated* version of its filesystem which
...
The unit test will now, at the *start* of every test, create a directory called
...
* `fake_mutex_init` resets the state of this module. This should be called in
  ...
  state will be carried over *between* tests, which isn’t desired.
...
To set CPPUTEST\_HOME and TARGET\_PLATFORM for your platform, edit the first few
...
Below is a more realistic report from the Memfault Public SDK[5](#fn:5).
...
In the example above, I had included a `fake_mutex.c` file *and* included the
...
> Testing with Mocks](/blog/unit-test-mocking).
...
1. [Test Driven Development](https://en.wikipedia.org/wiki/Test-driven_development) [↩](#fnref:0)
2. [2017 Embedded/EETimes Embedded Markets Study](https://www.embedded.com/electronics-blogs/embedded-market-surveys/4458724/2017-Embedded-Market-Survey) [↩](#fnref:1)
3. [Atlassian Code Coverage Overview](https://www.atlassian.com/continuous-delivery/software-testing/code-coverage) [↩](#fnref:2)
4. [littlefs Github page](https://github.com/ARMmbed/littlefs) [↩](#fnref:4)
5. [Memfault Public SDK](https://github.com/memfault/memfault-firmware-sdk) [↩](#fnref:5)
6. [AddressSanitizer](https://github.com/google/sanitizers/wiki/AddressSanitizer) [↩](#fnref:6)

![](/img/author/tyler.jpg)

[Tyler Hoffman](/authors/tyler) has worked on the embedded software teams at Pebble and Fitbit. He is now a founder at [Memfault](https://memfault.com).
