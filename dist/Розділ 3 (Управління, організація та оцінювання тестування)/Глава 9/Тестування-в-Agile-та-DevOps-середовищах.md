...
>**Ітеративна модель** Велика частина або повний цикл робіт проходить на кожній ітерації. На кожній ітерації можна аналізувати проміжні результати та вносити коригуючі зміни на наступних ітераціях. Кожна ітерація містить повний набір форм діяльності: від аналізу вимог до розгортання чергової функціональності.
...
| **Коригування вимог** | Дуже дорого            | Дешево, до наступної ітерації |
...
2. **Гнучкість до змін вимог навіть на пізніх стадіях**
...
|---|---|---|
| **Product Backlog** | Впорядкований список усіх вимог, функцій, дефектів; живий документ | QA читає, розуміє, може запитати уточнення у PO |
| **Sprint Backlog** | Підмножина Product Backlog для поточного спринту; завдання для виконання | QA бачить, що в спринті, планує тестування за цими завданнями |
| **Increment (готовий до випуску продукт)** | Готовий до використання результат спринту; додається до попередніх інкрементів | QA гарантує, що INCREMENT справді готовий до використання (протестований, приймається) |
...
| **Daily Standup** | 15 хвилин | DT | QA повідомляє: що було протестовано вчора, що планує тестувати сьогодні, які блокери; інші члени знають про статус |
...
>Зменшення числа паралельно виконуваних завдань сильно зменшує час виконання кожного окремого завдання. Немає потреби перемикати контекст між завданнями, відстежувати різні сутності, планувати їх і т.д.
...
**DevOps** — це не тільки набір інструментів (Docker, Kubernetes, Jenkins), а **культурна та організаційна трансформація**, яка об'єднує розробку (Dev), операції (Ops) та якість (QA) у єдину команду для прискорення та надійності поставки.
...
- QA стояли осторонь, перевіряли наприкінці
...
- Конфлікти: Ops каже "не можемо розгорнути, у вас непридатний код", Dev каже "не можемо змінити, ви нічого не розумієте у своїй роботі"

**DevOps філософія:**
- Розробник розуміє як його код запускається у production
- Ops інженер розуміє архітектуру коду
- QA **інтегрована з самого початку**, не наприкінці
...
- Більш ризиковано, вимагає хорошої автоматизації та моніторингу
...
- Програміст сфокусований на коді, пропускає перспективу користувача
- Тестування дефектів (edge cases) ігнорується
- Немає спеціаліста по якості
...
1. **Розширення мислення про якість**
   ...
   - QA змушує думати про граничні випадки, помилки, нетипові сценарії
...
3. **Спілкування та координація**
   - QA часто виявляє прогалини в комунікації
   - Наприклад: розробник неправильно зрозумів вимогу, QA це виявляє через завдання
...
- QA розуміє архітектуру системи, не тільки тестує з точки зору користувача
...
| **Sprint Retrospective** | 1-3 години | Обговорення ефективності тестування: що пішло добре, що можна поліпшити, які вузькі місця  (bottlenecks) можна покращити; планування інвестицій в автоматизацію; пропозиції по удосконаленню процесу | Відкритий діалог з командою; критичне мислення |
...
- QA: "Чудово. А що якщо користувач введе невалідну дату? Що якщо дата в минулому? Що якщо дата 100 років у майбутньому?"
...
- Фокус **НЕ на тому, хто допустив помилку**, а на **як запобігти цьому в майбутньому**
...
✅ **Правильно (Blameless):**
- На Retrospective: "Баг X проскочив через наш процес. Причина: немає інтеграційних тестів для цього сценарію. Рішення: додамо інтеграційний тест до CI/CD"
...
- **Паралельне тестування** — QA починає тестувати щойно код готовий, не чекаючи кінця спринту
...
*Вирішення:*
- Виявляється причина нестійкості (затримка в мережі, синхронізація БД, timing)
...
*Проблема:* Набір автотестів розростався багато років, тепер виконання займає 4 години. Розробники ігнорують результати.
...
1. **Розширених навичок** — технічні знання (тестування, автоматизація, DevOps), не тільки ручне тестування
...
**Найголовніше:** Agile та DevOps — це не про інструменти та процеси, а про **людей, їхнє мислення та культуру спілкування**.
