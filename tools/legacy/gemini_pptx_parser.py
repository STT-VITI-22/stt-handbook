...
PROMPT = """Ти - експерт з конвертації презентацій та лекцій (слайдів) у сучасний Markdown.
Твоє завдання: витягти текст з цього слайду з максимальним збереженням структури конспекту.
Суворо дотримуйся таких правил:

1. Текст: Розпізнай увесь корисний текст, зберігаючи логічні абзаци, списки та заголовки. Не пиши слова "Слайд 1" чи подібне. Форматуй як зв'язний конспект лекції.
2. КОЛОНТИТУЛИ ТА РЕКВІЗИТИ: Повністю ІГНОРУЙ номери слайдів, дату, імена викладачів, назву університету, кафедри або повторювану загальну назву дисципліни (наприклад, "Технології тестування..."), якщо це не головний заголовок найпершого титульного слайду.
...
5. ТАБЛИЦІ: Використовуй ТІЛЬКИ стандартний Markdown для таблиць. НІКОЛИ не використовуй команди LaTeX (як-от \\multicolumn) всередині таблиць. Markdown не підтримує об'єднання клітинок — просто залишай сусідні клітинки порожніми, якщо потрібно зімітувати colspan. Ніколи не дублюй одну й ту ж таблицю в HTML та Markdown.
...
7. ОПИС РАСТРОВИХ ЗОБРАЖЕНЬ: Якщо ти вирішив вставити посилання на растрову картинку, ОБОВ'ЯЗКОВО напиши розгорнутий текстовий опис її вмісту (alt-text) всередині квадратних дужок `![Детальний опис того, що саме намальовано чи показано на картинці](шлях)`.

Не додавай жодних своїх коментарів до тексту, повертай лише розпізнаний контент.
"""

NAME_PROMPT = """Проаналізуй титульні слайди цієї презентації.
Визнач тип заняття (лекція, групове заняття, практична) та його ДВОСТУПЕНЕВУ нумерацію (наприклад, 1.1, 2.4, 1.3).
Згенеруй унікальну назву файлу ЗА СУВОРИМ ФОРМАТОМ: Type_Number_TopicName.md
- Type: `lecture` (для лекцій), `group` (для ГЗ / групових занять), `pract` (для практичних).
- Number: точний номер зі слайду (наприклад `1_1`, `2_4` - обов'язково заміни крапку на підкреслення).
- TopicName: коротка назва теми англійською (2-3 слова, переклади з української).
Приклад: lecture_1_1_Software_Testing_Basics.md або group_1_3_Test_Metrics.md
Не використовуй кирилицю. Виведи ТІЛЬКИ назву файлу без жодних пояснень."""
...
def process_pdf(pdf_path, output_dir, keys):
    ...
    ignore_hashes = get_frequent_image_hashes(doc, threshold=3)
    if ignore_hashes:
